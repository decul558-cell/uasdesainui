...
<head>
    ...
    <style>
        :root {
            ...
            --text:#2C1B33;
            --text-muted:#7d6488;
            --shadow:0 4px 24px rgba(61,38,69,0.10);
            --shadow-lg:0 8px 40px rgba(61,38,69,0.22);
            --radius:12px;
        ...
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        #chat-toggle{width:54px;height:54px;border-radius:50%;background:var(--plum);border:none;cursor:pointer;color:white;font-size:1.3rem;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(61,38,69,0.4);transition:all 0.3s ease;position:relative;margin-left:auto;}
        ...
        #chat-send-btn{width:34px;height:34px;border-radius:50%;background:var(--plum);border:none;color:white;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:0.85rem;transition:background 0.2s;flex-shrink:0;}
        ...
        @media(max-width:768px){
            ...
            .navbar-pill.mobile-open{display:flex;flex-direction:column;position:fixed;top:74px;left:0;right:0;background:var(--plum-dark);padding:1.5rem;gap:0.5rem;border-radius:0;box-shadow:var(--shadow);z-index:999;}
            .navbar-pill.mobile-open a{text-align:center;padding:0.8rem;}
...
<nav class="navbar">
    ...
    <ul class="navbar-pill" id="navMenu" style="list-style:none;">
    ...
    <div class="navbar-actions">
        ...
        @php
            ...
            $unreadChat = $chatConv
                ? $chatConv->messages->where('sender_type', '!=', 'user')->where('is_read', false)->count()
                : 0;
    ...
    <button class="hamburger" onclick="document.getElementById('navMenu').classList.toggle('mobile-open')">
...
<script>
    ...
    function sendChatMessage() {
        ...
        .then(r => r.json())
        .then(() => { removeChatTyping(); pollChatMessages(); })
        .catch(() => removeChatTyping());
    }
    ...
    function appendChatMsg(type, text, avatar) {
        ...
        div.innerHTML = `
            ...
            <div>
                <div class="chat-bubble ${isMine ? 'user-bubble' : 'bot-bubble'}">${escapeChat(text)}</div>
    ...
    function pollChatMessages() {
        fetch(`{{ route('chat.poll') }}?after_id=${chatLastId}`)
            .then(r => r.json())
            .then(data => {
                if (data.messages && data.messages.length > 0) {
                    data.messages.forEach(msg => {
                        if (msg.sender_type !== 'user') {
                            const av = msg.sender_type === 'bot' ? '🤖' : 'A';
                            appendChatMsg(msg.sender_type, msg.message, av);
                        }
